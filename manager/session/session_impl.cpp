...
Status session_impl::mount(ServerContext *context, const empty *dummy_in, client_id *id)
{
	...
	if (next_id < (1 << CLIENT_BITS)) {
		id->set_id(next_id++);
		pool->write(obj_category::CLIENT, "next_id", reinterpret_cast<const char *>(&next_id), sizeof(uint64_t), 0);
	} else {
		id->set_id(-1);
	}
...
session_impl::session_impl(std::shared_ptr<rados_io> meta_pool) : pool(meta_pool)
{
	...
	if (pool->stat(obj_category::CLIENT, "next_id", size)) {
		pool->read(obj_category::CLIENT, "next_id", reinterpret_cast<char *>(&next_id), size, 0);
	} else {
		next_id = 1;
	}
}
